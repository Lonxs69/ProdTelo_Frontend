...
export const postsAPI = {
  ...
  promotePost: (postId, plan) => {
    if (!postId) throw new Error('postId es requerido');
    if (!plan || !['BASIC', 'PREMIUM'].includes(plan)) {
      throw new Error('plan debe ser BASIC o PREMIUM');
    }
    
    return apiRequest('/posts/payment/promote', {
      method: 'POST',
      body: JSON.stringify({ postId, plan })
    });
  },

  confirmPromotePost: (paymentId, postId, plan) => {
    ...
    return apiRequest(`/posts/payment/promote/${paymentId}/confirm`, {
      method: 'PUT',
      body: JSON.stringify({ postId, plan })
    });
  },
